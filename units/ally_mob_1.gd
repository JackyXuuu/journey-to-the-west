...
func _physics_process(_delta):
	if state == States.DYING:
		...
		if attack_target:
			...
			velocity = Vector2(sign(direction_x) * stats.sprint_speed, 0)
			animated_sprite.flip_h = velocity.x < 0
		else:
			animated_sprite.flip_h = direction < 0  # Ensure proper facing direction
			velocity = Vector2(direction * stats.sprint_speed, 0)
	move_and_slide()
